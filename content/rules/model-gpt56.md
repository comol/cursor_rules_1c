...
- **Do not re-read overlapping files** inside one task. `coding-standards.md` is an index — read the owner it points at, not the index again. Same for the verification trio (`verification-policy` → `verification-gates` → `verification-delivery`): load the stage you are in.
