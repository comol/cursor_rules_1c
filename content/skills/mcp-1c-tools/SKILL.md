...
| **1c-syntax-checker-mcp** | BSL syntax and style via BSL Language Server: `syntaxcheck` (code as text) and `syntaxcheck_file` (check a copied file by relative path, optionally line-filtered; prefer it over `syntaxcheck` when available) | [`docs/1c-syntax-checker-mcp.md`](docs/1c-syntax-checker-mcp.md) |
...
5. `1c-syntax-checker-mcp` — BSL syntax / style validation after edits (prefer `syntaxcheck_file` over `syntaxcheck` when it is exposed: read `.dev.env` `MCP_SyntaxChecker_PATH`, copy the file there, then pass the relative path).
